...
import { getZhibuReports, downloadReport, reviewReport } from '@/api/report'
...
const canReview = (row) => {
  return row.current_step === CURRENT_STEP.ZBSJ && row.status === REPORT_STATUS.PENDING
}
...
const handleApprove = async (row) => {
  try {
    ...
    // 执行通过审核
    await reviewReport(row.id, 'approve', '支部书记审核通过')
